...
if ~isempty(datmask)
  xi           = linspace(hlim(1), hlim(2), gridscale);   % x-axis for interpolation (row vector)
  yi           = linspace(vlim(1), vlim(2), gridscale);   % y-axis for interpolation (row vector)
  alpha_maskimage = griddata(chanX', chanY, double(datmask), xi', yi, 'nearest'); % interpolate the mask data
  alpha_maskimage(alpha_maskimage(:)==0)=0.4;
  if ~isempty(maskimage)
    alpha_maskimage=alpha_maskimage.*maskimage;
  end
end
...
if strcmp(style, 'surf') || strcmp(style, 'surfiso')
  ...
  h = surf(Xi-deltax/2, Yi-deltay/2, zeros(size(Zi)), Zi, 'EdgeColor', 'none', 'FaceColor', shading, 'AlphaData',alpha_maskimage,'FaceAlpha','flat');
  set(h, 'tag', tag);
  if ~isempty(parent)
    set(h, 'Parent', parent);
  end

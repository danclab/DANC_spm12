...
noiseFile = cfg_files;
noiseFile.tag = 'noiseFile';
noiseFile.name = 'M/EEG dataset to get noise from';
noiseFile.filter = 'mat';
noiseFile.num = [1 1];
noiseFile.help = {'Select the M/EEG mat file to get noise from.'};
...
isSNR.values = {setSNR, whitenoise, noiseFile};
isSNR.val = {setSNR};
...
if isfield(job.isSNR,'whitenoise'),
    ...
    SNRdB=[];
    Dnoise=[];
elseif isfield(job.isSNR,'setSNR'),
    ...
    SNRdB=[];
    whitenoisefT=[];
    Dnoise=spm_eeg_load(job.isSNR.noiseFile{1});
end;
...
if isfield(job.isinversion,'setsources'), %% defining individual sources
    ...
    [D,meshsourceind]=spm_eeg_simulate(D,job.prefix,job.isinversion.setsources.locs,simsignal,ormni,woi,whitenoisefT,SNRdB,trialind,mnimesh,dipfwhm,nAmdipmom,Dnoise);

...
for i=1:Ntrials
    ...
    if isempty(noiseD)
        ...
        N=pinknoise(size(Dnew(:,:,i),1),size(Dnew(:,:,i),2));
        Np=[zeros(size(Dnew(:,:,i),1),100) N zeros(size(Dnew(:,:,i),1),100)];
        Nf = ft_preproc_lowpassfilter(Np, Dnew.fsample, 50, 6,...
            'but', 'twopass', 'reduce');
        Nf=Nf(:,101:size(Nf,2)-100);
        Dnew(:,:,i)=Dnew(:,:,i)+Nf.*whitenoise;
